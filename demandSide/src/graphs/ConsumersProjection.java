...
public class ConsumersProjection {
	...
	public ConsumersProjection(Consumers consumers) {
		...
		space = new ContextSpace<Consumer>("ConsumersProjection", new SimpleCartesianAdder<Consumer>(),
	...
	public void update(Consumer c) {
		space.moveTo(c, margUtilToCoord(c.getRawWelfareParam()), MAX_Y / 2.0);
	}

	private double margUtilToCoord(double margUtilOfQuality) {
		return FastMath.min(margUtilOfQuality / getMaxUtilToDraw() * (MAX_X - MIN_X) + MIN_X, MAX_X);
	}
